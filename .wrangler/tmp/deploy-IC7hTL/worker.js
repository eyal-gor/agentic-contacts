...
// src/schema.ts
var ContactInput = external_exports.object({
  name: external_exports.string().min(1, "name is required"),
  emails: external_exports.array(external_exports.string().email()).default([]),
  phones: external_exports.array(external_exports.string()).default([]),
  ...
  // The message queued for this person, written ahead of time by whoever or
  // whatever prepares the day. Cleared when it's sent.
  draft: external_exports.string().nullable().optional(),
  // "Next talk" reminder: when to next reach out, and what about.
  // nullable so a PATCH can clear it by sending null.
  followUpAt: external_exports.string().nullable().optional(),
  followUpNote: external_exports.string().nullable().optional(),
  // Who created the record — see companies.addedBy.
  addedBy: external_exports.string().optional()
});
var ContactPatch = ContactInput.partial();
var CompanyInput = external_exports.object({
  name: external_exports.string().min(1, "name is required"),
  ...
  fitReason: external_exports.string().optional(),
  suggestedAngle: external_exports.string().optional(),
...
var CompanyPatch = CompanyInput.partial();
var InteractionInput = external_exports.object({
  channel: external_exports.enum(["call", "email", "meeting", "message", "note", "other"]).default("note"),
  summary: external_exports.string().min(1, "summary is required"),
  // ISO timestamp of when it happened; defaults to now on the server.
  occurredAt: external_exports.string().optional()
});
var ListInput = external_exports.object({
  ...
  offer: external_exports.string().nullable().optional(),
...
function definedEntries(patch) {
  return Object.entries(patch).filter(([, v]) => v !== void 0);
}
__name(definedEntries, "definedEntries");
function buildSet(patch, jsonFields = []) {
  const entries = definedEntries(patch);
  const clause = entries.map(([k]) => `${k} = ?`).join(", ");
  const values = entries.map(
    ([k, v]) => jsonFields.includes(k) ? toJson(v) : v
  );
  return { clause, values };
}
__name(buildSet, "buildSet");

// src/store.ts
var JSON_FIELDS = ["emails", "phones", "tags"];
...
async function create(db, input) {
  ...
  const contact = { id: crypto.randomUUID(), createdAt: now, updatedAt: now, ...input };
  await db.prepare(
...
async function update(db, id, patch) {
  ...
  const { clause, values } = buildSet(patch, JSON_FIELDS);
  const updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  const sql = clause ? `UPDATE contacts SET ${clause}, updatedAt = ? WHERE id = ?` : "UPDATE contacts SET updatedAt = ? WHERE id = ?";
...
__name(rowToInteraction, "rowToInteraction");
async function listForContact(db, contactId) {
  const { results } = await db.prepare("SELECT * FROM interactions WHERE contactId = ? ORDER BY occurredAt DESC").bind(contactId).all();
  return results.map(rowToInteraction);
}
__name(listForContact, "listForContact");
async function add(db, contactId, input) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const interaction = {
...
__name(all, "all");
async function lastByContact(db) {
...
__name(lastByContact, "lastByContact");
async function removeForContact(db, contactId) {
  const res = await db.prepare("DELETE FROM interactions WHERE contactId = ?").bind(contactId).run();
  return res.meta?.changes ?? 0;
}
__name(removeForContact, "removeForContact");
...
async function mintKey(env, name) {
...
__name(mintKey, "mintKey");
async function revokeKey(env, id) {
...
async function verifyMintedKey(env, key) {
  if (!key.startsWith("ac_")) return null;
  const row = await env.DB.prepare(
    "SELECT id FROM api_keys WHERE keyHash = ? AND revokedAt IS NULL"
  ).bind(await hashKey(key)).first();
  return row?.id ?? null;
}
__name(verifyMintedKey, "verifyMintedKey");
function stampUsage(env, id) {
  return env.DB.prepare("UPDATE api_keys SET lastUsedAt = ? WHERE id = ?").bind((/* @__PURE__ */ new Date()).toISOString(), id).run();
}
__name(stampUsage, "stampUsage");

// src/auth.ts
var requireApiKey = /* @__PURE__ */ __name(async (c, next) => {
  ...
  if (await verifySession(c.env, bearer)) return next();
  const minted = await verifyMintedKey(c.env, bearer);
  if (minted) {
...
contacts.get("/", async (c) => {
  ...
  const result = await list(c.env.DB, {
    q,
    tag,
    limit: limit ? Number(limit) : void 0,
    offset: offset ? Number(offset) : void 0
  });
  const lastMap = await lastByContact(c.env.DB);
  ...
  if (due) {
    ...
    else if (due === "today") cutoff = new Date((/* @__PURE__ */ new Date()).setHours(23, 59, 59, 999)).getTime();
    else if (due === "week") cutoff = now + 7 * 864e5;
    else if (due === "all") cutoff = Infinity;
    else cutoff = Date.parse(due) || now;
    enriched = enriched.filter((ct) => ct.followUpAt && Date.parse(ct.followUpAt) <= cutoff).sort((a, b) => Date.parse(a.followUpAt) - Date.parse(b.followUpAt));
  }
  ...
  if (sort === "recent" || sort === "stale") {
    ...
    enriched.sort((a, b) => {
      ...
      if (av === bv) return 0;
      if (!av) return stale ? -1 : 1;
      if (!bv) return stale ? 1 : -1;
      return (av < bv ? -1 : 1) * (stale ? 1 : -1);
    });
  }
...
contacts.post("/", async (c) => {
  ...
  return c.json(await create(c.env.DB, parsed.data), 201);
});
contacts.get("/:id", async (c) => {
  const found = await get(c.env.DB, c.req.param("id"));
  if (!found) return c.json({ error: "not found" }, 404);
  const lastInteraction = (await listForContact(c.env.DB, found.id))[0] ?? null;
  return c.json({ ...found, lastInteraction });
});
contacts.get("/:id/interactions", async (c) => {
  const found = await get(c.env.DB, c.req.param("id"));
  if (!found) return c.json({ error: "not found" }, 404);
  const items = await listForContact(c.env.DB, found.id);
...
contacts.post("/:id/interactions", async (c) => {
  ...
  if (!parsed.success) return c.json({ error: "invalid", issues: parsed.error.issues }, 400);
  return c.json(await add(c.env.DB, found.id, parsed.data), 201);
});
contacts.patch("/:id", async (c) => {
  const parsed = ContactPatch.safeParse(await c.req.json().catch(() => null));
  if (!parsed.success) return c.json({ error: "invalid", issues: parsed.error.issues }, 400);
  const updated = await update(c.env.DB, c.req.param("id"), parsed.data);
  return updated ? c.json(updated) : c.json({ error: "not found" }, 404);
});
contacts.delete("/:id", async (c) => {
  const id = c.req.param("id");
  const ok = await remove(c.env.DB, id);
  if (!ok) return c.json({ error: "not found" }, 404);
  await removeForContact(c.env.DB, id);
...
var memberTag = /* @__PURE__ */ __name((id) => `list:${id}`, "memberTag");
...
__name(rowToList, "rowToList");
async function all2(db) {
  const { results } = await db.prepare("SELECT * FROM lists ORDER BY createdAt DESC").all();
  return results.map(rowToList);
}
__name(all2, "all");
...
__name(get2, "get");
async function create2(db, name) {
  const base = slugify(name);
  let id = base;
  let n = 2;
  while (await get2(db, id)) id = `${base}-${n++}`;
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const list3 = { id, name, offer: null, perDay: 0, createdAt: now, updatedAt: now };
...
async function update2(db, id, patch) {
  const existing = await get2(db, id);
  if (!existing) return null;
  const sets = [];
  const binds = [];
  ...
  if (patch.offer !== void 0) {
    sets.push("offer = ?");
    binds.push(patch.offer);
  }
  if (patch.perDay !== void 0) {
    sets.push("perDay = ?");
    binds.push(patch.perDay);
  }
  const updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  if (sets.length) {
    await db.prepare(`UPDATE lists SET ${sets.join(", ")}, updatedAt = ? WHERE id = ?`).bind(...binds, updatedAt, id).run();
  }
  return get2(db, id);
}
__name(update2, "update");
async function remove2(db, id) {
...
// src/routes/lists.ts
var listsRoute = new Hono2();
listsRoute.use("*", requireApiKey);
async function members(db, id) {
  return list(db, { tag: memberTag(id), limit: 1e5 });
}
__name(members, "members");
listsRoute.get("/", async (c) => {
  const all3 = await all2(c.env.DB);
  const withCounts = await Promise.all(
    all3.map(async (l) => ({ ...l, memberCount: (await members(c.env.DB, l.id)).length }))
  );
...
listsRoute.post("/", async (c) => {
  ...
  if (!parsed.success) return c.json({ error: "invalid", issues: parsed.error.issues }, 400);
  return c.json(await create2(c.env.DB, parsed.data.name), 201);
});
listsRoute.get("/:id", async (c) => {
  const l = await get2(c.env.DB, c.req.param("id"));
  if (!l) return c.json({ error: "not found" }, 404);
  const mem = await members(c.env.DB, l.id);
...
listsRoute.patch("/:id", async (c) => {
  ...
  if (name === void 0 && offer === void 0 && perDay === void 0) {
    return c.json({ error: "nothing to update" }, 400);
  }
  const updated = await update2(c.env.DB, c.req.param("id"), { name, offer, perDay });
  return updated ? c.json(updated) : c.json({ error: "not found" }, 404);
});
listsRoute.delete("/:id", async (c) => {
  const id = c.req.param("id");
  const l = await get2(c.env.DB, id);
  if (!l) return c.json({ error: "not found" }, 404);
  const tag = memberTag(id);
  for (const m of await members(c.env.DB, id)) {
    await update(c.env.DB, m.id, { tags: (m.tags || []).filter((t) => t !== tag) });
  }
  await remove2(c.env.DB, id);
  return c.body(null, 204);
});
listsRoute.post("/:id/members", async (c) => {
  ...
  for (const cid of ids) {
    const contact = await get(c.env.DB, cid);
    if (!contact) continue;
    if (!(contact.tags || []).includes(tag)) {
      await update(c.env.DB, cid, { tags: [...contact.tags || [], tag] });
      added.push(cid);
    }
  }
...
listsRoute.delete("/:id/members/:contactId", async (c) => {
  ...
  const tag = memberTag(id);
  await update(c.env.DB, cid, { tags: (contact.tags || []).filter((t) => t !== tag) });
  return c.body(null, 204);
});

// src/companystore.ts
var JSON_FIELDS2 = ["domains", "tags"];
function rowToCompany(row) {
  return {
    ...
    sourceUrl: row.sourceUrl ?? void 0,
    fitReason: row.fitReason ?? void 0,
    suggestedAngle: row.suggestedAngle ?? void 0,
...
__name(rowToCompany, "rowToCompany");
async function list2(db, opts = {}) {
...
async function findByName(db, name) {
  const row = await db.prepare("SELECT * FROM companies WHERE lower(trim(name)) = ? LIMIT 1").bind(name.trim().toLowerCase()).first();
  return row ? rowToCompany(row) : null;
}
__name(findByName, "findByName");
async function create3(db, input) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const company = { id: crypto.randomUUID(), createdAt: now, updatedAt: now, ...input };
  await db.prepare(
...
async function update3(db, id, patch) {
  const existing = await get3(db, id);
  if (!existing) return null;
  const { clause, values } = buildSet(patch, JSON_FIELDS2);
  const updatedAt = (/* @__PURE__ */ new Date()).toISOString();
  const sql = clause ? `UPDATE companies SET ${clause}, updatedAt = ? WHERE id = ?` : "UPDATE companies SET updatedAt = ? WHERE id = ?";
  await db.prepare(sql).bind(...values, updatedAt, id).run();
  return get3(db, id);
}
__name(update3, "update");
async function upsertByName(db, input) {
  const existing = await findByName(db, input.name);
  if (!existing) return create3(db, input);
  const patch = {
    ...input,
    tags: [.../* @__PURE__ */ new Set([...existing.tags || [], ...input.tags || []])]
  };
  if (existing.addedBy) delete patch.addedBy;
  return await update3(db, existing.id, patch);
}
__name(upsertByName, "upsertByName");
async function remove3(db, id) {
...
var companiesRoute = new Hono2();
companiesRoute.use("*", requireApiKey);
async function members2(db, companyId, companyName) {
  const all3 = await list(db, { limit: 1e5 });
  const normalized = companyName.trim().toLowerCase();
  return all3.filter(
    (ct) => ct.companyId === companyId || (ct.company || "").trim().toLowerCase() === normalized
  );
}
__name(members2, "members");
companiesRoute.get("/", async (c) => {
  const { q, tag, status, limit, offset } = c.req.query();
  const result = await list2(c.env.DB, {
    q,
    tag,
    status,
    limit: limit ? Number(limit) : void 0,
...
companiesRoute.post("/", async (c) => {
  ...
  return c.json(upsert ? await upsertByName(c.env.DB, parsed.data) : await create3(c.env.DB, parsed.data), upsert ? 200 : 201);
});
companiesRoute.get("/:id", async (c) => {
  const found = await get3(c.env.DB, c.req.param("id"));
  if (!found) return c.json({ error: "not found" }, 404);
  const people = await members2(c.env.DB, found.id, found.name);
...
companiesRoute.patch("/:id", async (c) => {
  ...
  if (!parsed.success) return c.json({ error: "invalid", issues: parsed.error.issues }, 400);
  const updated = await update3(c.env.DB, c.req.param("id"), parsed.data);
  return updated ? c.json(updated) : c.json({ error: "not found" }, 404);
});
companiesRoute.delete("/:id", async (c) => {
  const ok = await remove3(c.env.DB, c.req.param("id"));
...
var PIPELINE_STAGES = ["lead", "contacted", "call", "trial", "proposal"];
var CHANNELS = ["call", "email", "meeting", "message", "note"];
function card(c) {
  return {
    id: c.id,
    name: c.name,
...
var TOOLS = [
  {
    ...
    description: "Search the address book by free text (name, company, title, notes, email, phone) and/or a tag. Returns compact contact cards including each contact's id \u2014 use get_contact for full history.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "get_contact",
    description: "Full dossier for one contact: every stored field plus the complete interaction history, newest first.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description: "Record that a conversation happened (call/email/meeting/message/note) and optionally set the next follow-up date. This is how outreach, replies and meetings enter the shared history.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "update_contact",
    description: "Update fields on an existing contact (title, company, tags, notes, follow-up\u2026). Only the fields you pass change.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  {
    name: "set_draft",
    description: "Write the message queued for this person \u2014 the one the daily queue shows with a copy button. Read the contact's history and notes first: a draft that could have been sent to anyone is worse than none. Pass an empty string to clear it.",
    inputSchema: {
      type: "object",
      properties: {
        contactId: { type: "string" },
  ...
  },
  {
    ...
    description: "The daily queue, same tiers as the app's Today view: follow-ups overdue, follow-ups due today, and pipeline contacts with no next step set.",
...
async function callTool(env, name, args) {
  ...
  switch (name) {
    ...
    case "update_contact": {
      ...
      const c = await update(db, id, patch);
      if (!c) throw new Error(`no contact with id ${id}`);
      return c;
    }
    case "set_draft": {
      ...
      const c = await update(db, id, { draft: text.trim() || null });
      if (!c) throw new Error(`no contact with id ${id}`);
      return { id: c.id, name: c.name, draft: c.draft };
    }
    case "who_to_contact_today": {
      const today = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
      const all3 = (await list(db, { limit: 1e5 })).filter((c) => !(c.tags ?? []).includes("do-not-contact"));
      const fu = /* @__PURE__ */ __name((c) => (c.followUpAt ?? "").slice(0, 10), "fu");
      const stage = /* @__PURE__ */ __name((c) => (c.tags ?? []).find((t) => t.startsWith("stage:"))?.slice(6), "stage");
      const overdue = all3.filter((c) => fu(c) && fu(c) < today);
      const due = all3.filter((c) => fu(c) === today);
      const adrift = all3.filter((c) => !fu(c) && PIPELINE_STAGES.includes(stage(c) ?? ""));
      return {
        date: today,
...
mcp.post("/", requireApiKey, async (c) => {
  ...
  if (id === void 0 || id === null) return c.body(null, 202);
  const reply = /* @__PURE__ */ __name((result) => c.json({ jsonrpc: "2.0", id, result }), "reply");
  const fail = /* @__PURE__ */ __name((code, message) => c.json({ jsonrpc: "2.0", id, error: { code, message } }), "fail");
  try {
    switch (method) {
      case "initialize":
        return reply({
...
app.post("/keys", requireApiKey, async (c) => {
  ...
  const { row, key } = await mintKey(c.env, name.trim());
  return c.json({ ...row, key }, 201);
});
app.delete("/keys/:id", requireApiKey, async (c) => {
  const ok = await revokeKey(c.env, c.req.param("id"));
  return ok ? c.json({ revoked: true }) : c.json({ error: "no such active key" }, 404);
});
app.get("/events", requireApiKey, async (c) => {
  const [all3, inter] = await Promise.all([
    list(c.env.DB, { limit: 1e5 }),
    all(c.env.DB)
  ]);
  const nameOf = Object.fromEntries(all3.map((ct) => [ct.id, ct.name]));
  const events = [];
  for (const ct of all3) {
