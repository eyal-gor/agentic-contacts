...
var ListInput = external_exports.object({ name: external_exports.string().min(1, "name is required") });
...
async function create2(db, name) {
  ...
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const list3 = { id, name, createdAt: now, updatedAt: now };
...
async function rename(db, id, name) {
...
listsRoute.patch("/:id", async (c) => {
  ...
  const updated = await rename(c.env.DB, c.req.param("id"), parsed.data.name);
  return updated ? c.json(updated) : c.json({ error: "not found" }, 404);
});
...
async function upsertByName(db, input) {
  ...
  return await update2(db, existing.id, patch);
}
...
companiesRoute.patch("/:id", async (c) => {
  ...
  if (!parsed.success) return c.json({ error: "invalid", issues: parsed.error.issues }, 400);
  const updated = await update2(c.env.DB, c.req.param("id"), parsed.data);
  return updated ? c.json(updated) : c.json({ error: "not found" }, 404);
});
