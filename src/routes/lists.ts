...
async function members(db: D1Database, id: string) {
  return store.list(db, { tag: lists.memberTag(id), limit: 100_000 });
}

listsRoute.get("/", async (c) => {
  const all = await lists.all(c.env.DB);
  const withCounts = await Promise.all(
    all.map(async (l) => ({ ...l, memberCount: (await members(c.env.DB, l.id)).length })),
  );
...
listsRoute.post("/", async (c) => {
  ...
  return c.json(await lists.create(c.env.DB, parsed.data.name), 201);
});

listsRoute.get("/:id", async (c) => {
  const l = await lists.get(c.env.DB, c.req.param("id"));
...
listsRoute.delete("/:id", async (c) => {
  ...
  const l = await lists.get(c.env.DB, id);
  if (!l) return c.json({ error: "not found" }, 404);
  ...
  for (const m of await members(c.env.DB, id)) {
    await store.update(c.env.DB, m.id, { tags: (m.tags || []).filter((t) => t !== tag) });
  }
  await lists.remove(c.env.DB, id);
...
listsRoute.post("/:id/members", async (c) => {
  ...
  for (const cid of ids) {
    const contact = await store.get(c.env.DB, cid);
    if (!contact) continue;
    if (!(contact.tags || []).includes(tag)) {
      await store.update(c.env.DB, cid, { tags: [...(contact.tags || []), tag] });
      added.push(cid);
    }
  }
...
listsRoute.delete("/:id/members/:contactId", async (c) => {
  ...
  const tag = lists.memberTag(id);
  await store.update(c.env.DB, cid, { tags: (contact.tags || []).filter((t) => t !== tag) });
  return c.body(null, 204);
});
